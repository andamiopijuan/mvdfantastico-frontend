...
export default function CurrentEditionHighlight({ edition }: CurrentEditionHighlightProps) {
  ...
  if (typeof window === "undefined") console.log("[CurrentEditionHighlight] posterSrc:", posterSrc);
  const description =
    locale === "en"
          ...
          <div className="md:col-span-8 pt-2">
            ...
            {description && (
              <div className="mb-10 max-w-lg space-y-4">
                {description.split("\n\n").map((para, i) => (
                  <p key={i} className="font-sans text-sm text-text-secondary leading-relaxed font-light">
                    {para}
                  </p>
                ))}
              </div>
